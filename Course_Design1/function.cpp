	...
	while (1) {
		while (MouseHit()) {
			...
			if (msg.uMsg == WM_LBUTTONDOWN && msg.x >= 816 && msg.x <= 993 && msg.y >= 240 && msg.y <= 268) {
				...
				wchar_t no[2];
				InputBox(no, 2, _T("Number"), _T("编号"), NULL, 0, 0, true);
				MessageBox(hwnd, L"删除用户成功！", _T("恭喜恭喜"), MB_OK);
				int n;
	...
	while (1) {
		while (MouseHit()) {
			...
			if (msg.uMsg == WM_LBUTTONDOWN && msg.x >= 812 && msg.x <= 1024 && msg.y >= 238 && msg.y <= 272) {
				...
				MessageBox(hwnd, L"成功添加新用户", _T("恭喜恭喜"), MB_OK);
...
void sort_user_panel() {
	...
	outtextxy(691, 293, _T("\t编号      \t账号         \t密码"));
	ifstream in("Data/user.txt", std::ios::in);
	int n;
	in >> n;
	for (int i = 1, y = 313; i <= n; i++, y += 20) {
		wchar_t user_Account[20];
		wchar_t user_Password[20];
		wchar_t user_information[100];
		int no;
		string account, password;
		in >> no >> account >> password;
		wchar_t* wideAccount = stringToWideCharArray(account);
		wchar_t* widePassword = stringToWideCharArray(password);
		_stprintf_s(user_information, _T("%-5d      %-10s      %-20s "), no, wideAccount, widePassword);
		outtextxy(691, y, user_information);
	}
	...
	while (1) {
		while (MouseHit()) {
			msg = GetMouseMsg();
			if (msg.uMsg == WM_LBUTTONDOWN && msg.x >= 813 && msg.x <= 970 && msg.y >= 240 && msg.y <= 279) {
				user_manager_panel();
				goto label;
			}
		}
...
void user_System_Login() {
		...
		hwnd = GetHWnd();
		SetWindowText(hwnd, _T("电子投票系统 2.0"));
		fstream test("Data/user.txt", std::ios::in);
		if (!test.is_open()) {
			int i = MessageBox(hwnd, _T("文件打不开"), _T("没有用户存在,先用管理员生成！"), MB_OK);
			if (i == true)
				select_Identity();
		}
		else {
